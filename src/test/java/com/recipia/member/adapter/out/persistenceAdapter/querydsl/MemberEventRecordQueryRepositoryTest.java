...
class MemberEventRecordQueryRepositoryTest extends TotalTestSupport {
    ...
    @DisplayName("[happy] memberId, snsTopic이 동일한 이벤트중 가장 최근에 발행된 이벤트를 published = true로 업데이트 성공")
    @Test
    void latestEventChangePublishedToTrueSuccess() {
        ...
        // when
        Long updatedCount = sut.changePublishedToTrue(memberId, snsTopic);

        // then
        assertEquals(1L, updatedCount); // 하나의 레코드가 업데이트 되었는지 확인
    }

    @DisplayName("[bad] memberId, snsTopic이 동일한 이벤트중 가장 최근에 발행한 이벤트중에서 false인 데이터가 없을때 published = true 업데이트 실패")
    @Test
